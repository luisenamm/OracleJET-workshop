...
@include module-include-once("common.formcontrolcommon") {
  ...
  */
  ...
  .oj-text-field-container.oj-text-field-has-start-slot .oj-text-field-input {
    padding: 0 var(--oj-text-field-icon-to-text-padding);
  }
  ...
  .oj-text-field.oj-text-field-label-inside .oj-text-field-start .oj-button {
    margin-bottom: 0;
    @include oj-ltr() {
      margin-left: calc((var(--oj-text-field-inside-edge-height) - var(--oj-button-height)) / 2);
    }
    @include oj-rtl() {
      margin-right: calc((var(--oj-text-field-inside-edge-height) - var(--oj-button-height)) / 2);
    }
  }
  ...
  // For the input form components, we hide the helphints when not in focus.
  // When we animate helphints, we don't remove oj-focus styleclass until animation is done
  // so that it will animate out before the display:none.
  ...
  // We want the Required dom to fade away when the user types in a value and blurs.
  // It's ok that it does not fade away when we get a message or hint since those
  // are fading in. In those cases we set display to none on the required container.
  .oj-form-control:not(.oj-has-no-value) .oj-required-inline-container {
    opacity: 0;
    transition: opacity 0.2s cubic-bezier(0.4, 0, 0.2, 1);
  }

  .oj-form-control.oj-has-no-value .oj-required-inline-container {
    opacity: 1;
  }
